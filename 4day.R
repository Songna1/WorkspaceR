...
#
# 변수명 부여 방식
numberValue <- 1            # camel 표기법    <처음엔 소문자 지만 중간에 대문자를 사용(낙타등)
str_value <- "R Language"   # snake 표기법    <가운데 언더 바 (뱀)
booleanvalue <- TRUE        # 일반 표기법     변수는 소문자를 사용
...
#cat() : 여러 내용을 출력할 수 있고, ★자동 줄바꿈이 일어나지 않는★표준 출력 장치에 
#         출력하는 함수
...
print( '----------------------------------------')
cat('Numeric number : ', numberValue, '\t',    # 줄바꿀 땐 콤마 , 쓰기
    'string : ', str_value, '\t',
...
#출력(output, print)--- memory내용을 output device에 전송
...
# }
#
i = 1   # 반복 제어 변수 초기화  (i, j, k 등등 임의로사용가능 )
while ( i <= 10 ) {   # 반복 제어 변수 조건 검사, 참인동안 반복 ///// i가 10보다 작은거만 반복 i가 11되었을때 끝
    print( '*' )                  
    i <- i + 1    # 반복 제어 변수 값 변경, 누적    ////i가 11되었을때 끝
}
...
while ( i <= 10 ) {   # 반복 제어 변수 조건 검사, 참인동안 반복 ///// i가 10보다 작은거만 반복 i가 11되었을때 끝
    print( '*' )                  
    i <- i + 2   # 반복 제어 변수 값 변경, 누적    ////i가 11되었을때 끝
}
...
#2.4 함수
#
# 함수(Function) : 단위 기능을 수행하는 코드 집합 ///항상 괄호가 붙어야 함 괄호 없을 시 변수 
#
# 함수 종류
