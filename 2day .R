...
missing <- NA   # NA : 결측치(missing value) <<잘못된 값
chrnum <- '10'  # 영숫자 <<숫자 모양을 한 '문자'
...
#
#벡터간 연산
#두 벡터간 길이가 동일해야함. 같은 인덱스끼리 계산됨. 대부분 합이나 곱
x <- c(1, 2, 3)
y <- c(4, 5, 6)
x+y          #벡터 합 계산
x*y          #벡터 곱 계산
...
mean(v)            #평균
mean(v[1:5])
median(v)          #중앙값 <<데이터 왜곡이 덜 함.
median(v[1:5])
max(v)             #최대값
min(v)             #최소값
sort(v, decreasing = FALSE)  #정렬(줄 세우기-오름차순)
sort(v, decreasing = TRUE)                  #내림차순
...
#관계 연산자 이용한 관계 연산 <<<관계연산은 대소 비교
number1 > number2
number1 <= number2

# 관계/ 논리 연산자 이용한 논리 연산 <<<그리고, 또는, 부정
...
v >= 5
v[v>5]
sum(v>5) #0000011111 < false / true 구분해서 합계 
v==5     

condition <- v>5 & v<8    #따로 계산하고 공통되는 것
condition
v[condition]
